...
import { REQUIRES_3D, extractLastUserText, elapsed, is3DItemType, AGENT_KEYWORDS } from "../../utils/agentHelpers";
...
const ContentSearchState = Annotation.Root({
...
type ContentSearchStateType = typeof ContentSearchState.State;
...
const createContentSearchGraph = () => {
  async function contentSearchNode(s: ContentSearchStateType, config?: RunnableConfig) {
    const text = extractLastUserText(s);
    await sendTraceMessage({ text: "ContentSearch: processing request" }, config);
