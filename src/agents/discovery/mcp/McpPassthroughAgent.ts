...
export function registerMcpPassthroughAgent(
  ...
  const createGraph = () => {
    ...
    function respondNode(agentState: any) {
      ...
      // Build chat response: prefer LLM text, fall back to tool output summary
      let text = contentToText(lastAiMessage?.content);
      if (!text && toolOutputs.length > 0) {
        // LLM didn't produce a text summary — show tool outputs directly
        text = toolOutputs
          .map((output) => {
            try {
              const parsed = JSON.parse(output);
              // If it's structured JSON, format key fields
      ...
      if (!text) {
        text = `The ${ctx.serverName || "MCP"} server did not return a response.`;
      }
