...
const LoadLayerState = Annotation.Root({
...
type LoadLayerStateType = typeof LoadLayerState.State;

// ── Agent graph ──────────────────────────────────────────────────────────────

const createLoadLayerGraph = () => {
  async function loadLayerNode(s: LoadLayerStateType, config?: RunnableConfig) {
      ...
      try {
        ...
        const result = await withTimeout(
          locator.addressToLocations(
            ...
            { address: { SingleLine: stripped || text.replace(/\b(zoom|go|fly|navigate)\s*(to|at|in)?\s*/gi, "").trim() }, maxLocations: 1 }
          ),
          10000,
          "Geocode place name"
        );
        ...
        if (candidate?.location) {
          await activeView.goTo(
            { target: candidate.location, zoom: candidate.attributes?.Addr_type === "Locality" ? 12 : 15 },
            { duration: 2000 }
          );
          return { outputMessage: `Zoomed to **${candidate.address || stripped}**.` };
        }
      } catch (err: any) {
        console.warn("[LoadLayer] Geocode failed:", err?.message);
      }
      return { outputMessage: `Could not find a layer or location named "${stripped}".` };
    }
